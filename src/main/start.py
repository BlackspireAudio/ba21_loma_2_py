...
""""Different phase shifters with configurations. Can process multiple configs successively."""
configs = {
...
def process_sine(info: TrackInfo, time, frequencies, zero_padding=False):
    """"Creates a sine wave to process it with the given signal processors"""
...
def process_wav(info: TrackInfo, name, zero_padding=False):
    """"Reads a WAVE file to process it with the given signal processors"""
    info.name = name
    info.setup(zero_padding)
